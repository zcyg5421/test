...
struct Schedule * DSRM(struct Task tl[], int num_task, int sim_time){
/*
...
*/
    ...
    float Uti_P=1;	//Utilization paramter
    float UtiMax=0;	//Maximum utilization
    float Uti=0;	//Utilization
    float Cs=0;		//deferrable server capacity
    float Ts=0;		//deferrable server period

    for(int i = 0; i< num_task; i++){
	    Uti_P *= tl[i].C/tl[i].T+1;
    }	//Calculating P for deferrable server

    UtiMax = (2-Uti_P)/(2*Uti_P-1);
    Uti = UtiMax;
    ...
    for(int i=0; i<num_task; i++){
	    if(i==num_task-1)
		    ...
		    temp1=tl[i].T;
		    temp2=tl[i+1].T;
		    Ts=(temp1>temp2)?temp2:temp1;
	    }
    }	//Pick the minimum Ti

    Cs=Ts*Uti;
    /* Finish deferrable server parameter definition */
